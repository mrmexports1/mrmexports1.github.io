...
def build_contact_jscripts(outfile):
    replace_text_with_file_context({'contact_jscript':CONTACT_JSCRIPT_FILE}, outfile)

def build_contact_content(outfile):
    replace_text_with_file_context({'contact_content':CONTACT_CONTENT_FILE}, outfile)
...
def main():
    ...
    #if not opt.outfile:
    outfile = '{}/contactus/contactus.html'.format(PROJ_ROOT)

    copyfile(CONTACTPAGE_TEMPLATE, outfile)

    build_site_head(outfile)
    build_site_navigation('Contact Us', outfile)
    build_site_footer(outfile)
    build_google_anlytics(outfile)

    build_contact_jscripts(outfile)
    build_contact_content(outfile)

    replace_text({'title': 'Contact Us | ' + PROJ_TITLE}, outfile)
